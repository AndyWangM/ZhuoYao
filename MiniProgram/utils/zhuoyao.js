...
(function (ZhuoYao) {
  var Common = function () {
    ...
    function Utils() { } Utils.hash = function (input) { var hash = 5381; var i = input.length - 1; if (typeof input == "string") for (; i > -1; i--)hash += (hash << 5) + input.charCodeAt(i); else for (; i > -1; i--)hash += (hash << 5) + input[i]; var value = hash & 2147483647; var retValue = ""; do retValue += this.I64BIT_TABLE[value & 63]; while (value >>= 6); return retValue }; Utils.utf8ByteToUnicodeStr = function (utf8Bytes) {
      ...
      function (spriteList) { this.spriteHash = new HashMap; this.spriteNameHash = new HashMap; for (var _i = 0, spriteList_1 = spriteList; _i < spriteList_1.length; _i++) { var spriteInfo = spriteList_1[_i]; this.spriteHash.put(spriteInfo.Id, spriteInfo); this.spriteNameHash.put(spriteInfo.Name, spriteInfo.Id) } }; Utils.getSpriteList = function () {
        if (!this.spriteHash || !this.spriteNameHash) {
        this.spriteHash = new HashMap; this.spriteNameHash = new HashMap; var spriteList = Utils.getStorage("SpriteList"); for (var _i = 0, spriteList_2 = spriteList; _i <
          spriteList_2.length; _i++) { var spriteInfo = spriteList_2[_i]; this.spriteHash.put(spriteInfo.Id, spriteInfo); this.spriteNameHash.put(spriteInfo.Name, spriteInfo.Id) }
        } return this.spriteHash
      }; Utils.getSpriteNameHash = function () {
        if (!this.spriteHash || !this.spriteNameHash) {
        this.spriteHash = new HashMap; this.spriteNameHash = new HashMap; var spriteList = Utils.getStorage("SpriteList"); for (var _i = 0, spriteList_3 = spriteList; _i < spriteList_3.length; _i++) {
          var spriteInfo = spriteList_3[_i]; this.spriteHash.put(spriteInfo.Id, spriteInfo);
    ...
    Utils.getSpriteSearchNameFilter = function () { var spriteList = this.getStorage("SpriteList"); var arr = []; for (var i = 0; i < spriteList.length; i++)if (spriteList[i].Checked) arr.push(spriteList[i].Name); return arr }; Utils.getTempResults = function () { return this.tempResults }; Utils.tempResults = new HashMap; Utils.I64BIT_TABLE = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-".split(""); return Utils
  }(); ZhuoYao.Utils = Utils
})(ZhuoYao || (ZhuoYao = {})); var ZhuoYao;
(function (ZhuoYao) {
  var Socket = function () {
    function Socket(content) { this.requestIds = []; this.isOpen = false; this.messageQueue = []; this.isReceiving = false; this.requestResult = new RequestResult; this.content = content } Socket.prototype.initSocket = function () {
      var that = this; that.connectSocket(); wx["onSocketOpen"](function (t) { that.socketConnectedCallback(t) }); wx["onSocketError"](function (e) { console.log("WebSocket\u8fde\u63a5\u6253\u5f00\u5931\u8d25\uff0c\u8bf7\u68c0\u67e5\uff01"); setTimeout(function () { that.connectSocket() }, 1E3) });
      wx["onSocketClose"](function (e) { console.log("WebSocket \u5df2\u5173\u95ed\uff01"); setTimeout(function () { that.connectSocket() }, 500) }); wx["onSocketMessage"](function (t) { that.recMessage(t) })
    }; Socket.prototype.connectSocket = function () { wx["connectSocket"]({ url: "wss://publicld.gwgo.qq.com?account_value=0&account_type=0&appid=0&token=0" }) }; Socket.prototype.socketConnectedCallback = function (t) { console.log("WebSocket\u8fde\u63a5\u5df2\u6253\u5f00\uff01"); this.isOpen = true }; Socket.prototype.sendMessage = function (str) {
      var that =
        this; that.sendSocketMessage(str)
    }; Socket.prototype.sendSocketMessage = function (str, callback) { var that = this; wx["sendSocketMessage"]({ data: ZhuoYao.Utils.str2ab(str), success: function (n) { console.log("\u53d1\u9001\u670d\u52a1\u5668\u6210\u529f", str) }, fail: function (n) { console.log("\u53d1\u9001\u670d\u52a1\u5668\u5931\u8d25") } }) }; Socket.prototype.recMessage = function (e) {
      var that = this; var str = ZhuoYao.Utils.utf8ByteToUnicodeStr((new Uint8Array(e.data)).slice(4)); if (str.length > 0) {
        console.log("\u6536\u5230\u670d\u52a1\u5668\u6d88\u606f");
        var obj = JSON.parse(str); if (obj["retcode"] != 0) wx["hideLoading"](); var id = that.getRequestTypeFromId(obj["requestid"]); if (id == "10041") this.getVersionFileName(obj["filename"]); else if (obj.sprite_list) {
          var spriteResult = that.requestResult.getSpriteResult(obj); for (var _i = 0, _a = spriteResult.sprite_list; _i < _a.length; _i++) {
            var aliveSprite = _a[_i]; if (!aliveSprite.sprite) continue; var sprite = aliveSprite.sprite; if (sprite) {
              var spriteNameFilter = ZhuoYao.Utils.getSpriteSearchNameFilter(); if (spriteNameFilter.length > 0) for (var _b =
                0, spriteNameFilter_1 = spriteNameFilter; _b < spriteNameFilter_1.length; _b++) {
                  var spriteName = spriteNameFilter_1[_b]; if (sprite.Name == spriteName) {
                    var latitude = aliveSprite.latitude.toString().substr(0, 2) + "." + aliveSprite.latitude.toString().substr(2); var longtitude = aliveSprite.longtitude.toString().substr(0, 3) + "." + aliveSprite.longtitude.toString().substr(3); var resultObj = { "name": aliveSprite.sprite.Name, "latitude": latitude, "longtitude": longtitude, "lefttime": aliveSprite.getLeftTime() }; var hashStr = "" + aliveSprite.sprite_id +
                      aliveSprite.latitude + aliveSprite.longtitude + aliveSprite.gentime + aliveSprite.lifetime; var hashValue = ZhuoYao.Utils.hash(hashStr); if (!ZhuoYao.Utils.getTempResults().containsKey(hashValue)) ZhuoYao.Utils.getTempResults().put(hashStr, resultObj)
                  }
      ...
      var that = this; var g = (new Date).getTime() % 1234567; switch (n) {
        case "1001": that.requestIds[0] = g; break; case "1002": that.requestIds[1] = g; break; case "1003": that.requestIds[2] = g; break; case "10040": that.requestIds[3] =
          g; break; case "10041": that.requestIds[4] = g
      }return g
      ...
      var that = this; if (that.requestIds[0] == n) return "1001"; else if (that.requestIds[1] == n) return "1002"; else if (that.requestIds[2] == n) return "1003"; else if (that.requestIds[3] ==
        n) return "10040"; else if (that.requestIds[4] == n) return "10041"; else return 0
      ...
      var that = this; console.log("\u5b58\u5728\u65b0\u7248\uff0c\u4e0b\u8f7d\u6210\u529f" + i); wx["downloadFile"]({
        "url": "https://hy.gwgo.qq.com/sync/pet/config/" + i, "success": function (s) {
          if (200 === s["statusCode"]) {
            ...
            var spriteList = l["Data"]; ZhuoYao.Utils.setSpriteList(spriteList); ZhuoYao.Utils.setSpriteHash(spriteList)
  ...
  }(); ZhuoYao.Socket = Socket; var RequestResult = function () {
    function RequestResult() { } RequestResult.prototype.getSpriteResult =
      function (result) { return new SpriteResult(result) }; return RequestResult
  }(); var AliveSprite = function () {
    function AliveSprite(obj) { this.gentime = obj["gentime"]; this.latitude = obj["latitude"]; this.lifetime = obj["lifetime"]; this.longtitude = obj["longtitude"]; this.sprite_id = obj["sprite_id"]; this.initSprite() } AliveSprite.prototype.getLeftTime = function () { var that = this; var time = that.gentime + that.lifetime; var leftTime = time - (new Date).getTime() / 1E3; return that.formatTime(leftTime.toFixed(0)) }; AliveSprite.prototype.initSprite =
      function () { var spriteList = ZhuoYao.Utils.getSpriteList(); this.sprite = spriteList.get(this.sprite_id) }; AliveSprite.prototype.formatTime = function (timeStr) { var time = Number(timeStr); var hour = parseInt((time / 3600).toString()); time = time % 3600; var minute = parseInt((time / 60).toString()); time = time % 60; var second = parseInt(time.toString()); return [hour, minute, second].map(function (n) { var num = n.toString(); return num[1] ? num : "0" + num }).join(":") }; return AliveSprite
  }(); var SpriteResult = function () {
    function SpriteResult(obj) {
    this.end =
      obj["end"]; this.packageNO = obj["packageNO"]; this.requestid = obj["requestid"]; this.retcode = obj["retcode"]; this.retmsg = obj["retmsg"]; this.sprite_list = []; for (var i = 0; i < obj["sprite_list"].length; i++) { this.sprite_list[i] = new AliveSprite(obj["sprite_list"][i]); if (!this.sprite_list[i]) console.log(1) }
    } return SpriteResult
  }()
